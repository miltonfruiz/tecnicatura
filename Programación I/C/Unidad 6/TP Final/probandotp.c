...
void cargaArticulos(char arreglo1[][6][50], int total, char arreglo2[][2][50]);
...
char* buscoCodigo(char codigo[], char arreglo[][2][50], int limite);
int validoEntero(const char ingresado[]);
...
// ------------------------------------------------------------------------------------------
void ingreso() {
    char n[10], articulos[][][50], rubro[6][2][50];
    int cantidad, fil;
    ...
    fil = atoi(n);
    char cargaArticulos(char arreglo1[fil][6][50]);
    cargaArticulos(articulos, fil, rubro);
}

// Implementación de la función cargaRubros
void cargaRubros(char arreglo[5][2][50]) {
...
void cargaArticulos(char arreglo1[fil][6][50], int total, char arreglo2[5][2][50]) {
...
void ingresoCodigo(char arreglo1[][6][50], char arreglo2[5][2][50], int f) {
    char codigo[50];
    char cod[50], nAticulos[6];

    do {
        do {
            printf("\033[H\033[J"); // Limpiar pantalla (ANSI escape code)
            printf("\n>>> Carga de datos: Artículo N° %d\n\n", f);
            printf("    # Ingrese código: (3 dígitos)\n");
            scanf("%s", cod);
            if (strlen(cod) != 3) {
        ...
        strcpy(codigo, buscoCodigo(cod, arreglo2, 5));
        if (strcmp(buscoCodigo(cod, arreglo2, 5), "-1") != 0) {
        printf("¡Ese código no existe!\n");
}       else {
            do {
                printf("\033[H\033[J"); // Limpiar pantalla (ANSI escape code)
                printf("\n>>> Carga de datos: Artículo N° %d\n\n", f);
                printf("    # Ingrese número de artículo: (5 dígitos)\n");
                scanf("%s", nArticulos);
                if (strlen(nArticulos) != 5) {
                    printf("¡Número de Artículo Ingresado Incorrecto!\n");
                }
            } while (strlen(nArticulos) != 5);
        }
...
char* buscoCodigo(char codigo[], char arreglo[][1][50], int limite) {
    int i = 0;
    char* encontrado = NULL;
    while (i < limite && strcmp(codigo, arreglo[i][0]) != 0) {
        i++;
    }
    if (i < limite) {
        encontrado = arreglo[i][0];
    } else {
        encontrado = "-1";
    }
    return encontrado;
...
void muestraCodigo(char arreglo1[][6][50], int total) {
    ...
    do {
        ...
        // Buscar el código en el arreglo1
        strcpy(busco, buscoCodigo(cod, arreglo1, total));

        // Verificar si el código existe en el arreglo1
        if (strcmp(busco, "-1") == 0) {
            ...
            printf("\n * Pulse una tecla para continuar...\n");
            getchar(); // Capturar la tecla Enter
...
// Implementación de la función ordenDescendente
// ------------------------------------------------------------------------------------------
void ordenDescendente(char arreglo[][6][50], int fila) {
    int i, j, k;
    float q1, q2, q3, q4;
